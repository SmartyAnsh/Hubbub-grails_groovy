...
<div class="fieldcontain ${hasErrors(bean: userDataInstance, field: 'profile', 'error')} ">
	...
	<g:select id="profile" name="profile.id" from="${hubbub.ProfileOfUser.list()}" optionKey="id" value="${userDataInstance?.profile?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: userDataInstance, field: 'following', 'error')} ">
	...
	<g:select name="following" from="${hubbub.UserData.list()}" multiple="multiple" optionKey="id" size="5" value="${userDataInstance?.following*.id}" class="many-to-many" noSelection="['null': '']"/>
</div>
...
<g:each in="${userDataInstance?.posts?}" var="p">
    <li><g:link controller="postOfUser" action="show" id="${p.id}">${p?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="postOfUser" action="create" params="['userData.id': userDataInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'postOfUser.label', default: 'PostOfUser')])}</g:link>
</li>
</ul>
...
<g:each in="${userDataInstance?.tags?}" var="t">
    <li><g:link controller="taggingOfUser" action="show" id="${t.id}">${t?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="taggingOfUser" action="create" params="['userData.id': userDataInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'taggingOfUser.label', default: 'TaggingOfUser')])}</g:link>
</li>
</ul>
